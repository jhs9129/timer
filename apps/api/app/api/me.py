...
from app.schemas import MeOut, MePatch, VillageSummary
...
from app.services.villages import level_for, village_for_user
...
async def build_me(db: AsyncSession, user: User) -> MeOut:
    ...
    return MeOut(
        ...
        village=VillageSummary(
            slug=village.slug, name=village.name, xp=village.xp, level=level.level
        ),
    )
